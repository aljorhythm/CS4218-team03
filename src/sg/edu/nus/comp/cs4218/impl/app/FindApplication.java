...
public class FindApplication implements FindInterface {
    ...
    private String getArguments(String[] args, ArrayList<String> folderNames) throws Exception {
        ...
        for (String s : args) {
            if (!s.isEmpty() && s.charAt(0) == CHAR_FLAG_PREFIX) {
                if (s.equals(FILE_IDENT)) { // next arg is filename
                    isFileName = true;
                    continue;
                } else {
                    throw new Exception(WRONG_FLAG_SUFFIX);
                }
            }
            ...
            if (isFileName) {
                if (fileName.isEmpty()) {
                    ...
                    throw new Exception(MULTIPLE_FILES);
                }
            } else {
    ...
    private String findInFolders(String fileName, String... folderName) throws Exception {
        ...
        for (String f : folderName) {
            ...
            if (!folder.exists() || !folder.isDirectory()) {
                stringJoiner.add("find: " + f + ": " + FOLDER_NOT_FOUND);
                continue;
            }

            if (f.lastIndexOf(CHAR_FILE_SEP) != -1 && (f.lastIndexOf(CHAR_FILE_SEP) + 1 < f.length())) {
